    ...
    def update_plot(self):
        ...
        for i in range(2):
            ...
            if read_data != b'':
                ...
                self.x_data.append(self.x_data[-1]+worktime/2)
                
                if self.secondTime==int(self.x_data[-1]+(worktime)/2):
        ...
        palette = QPalette()
        palette.setColor(QPalette.Base, QColor('green'))  # Arka plan rengi
        palette.setColor(QPalette.Text, QColor('black'))  # Metin rengi
        
        if max(self.y_data)>=55000:
            self.joinAkımText.setPalette(palette)
            self.joinAkımValue.setText(str(round(max(self.y_data)/1000,2))+" mA")
