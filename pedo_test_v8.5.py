...
import os
import time
from PyQt5.QtWidgets import QApplication, QMainWindow, QHBoxLayout,  QLineEdit, QLabel, QWidget, QVBoxLayout, QPushButton, QSpacerItem, QSizePolicy, QComboBox
...
def on_message(client, userdata, message):
    ...
    received_messages=str(json.loads(message.payload.decode())['deviceName'])
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        vBoxTimeChoose.addWidget(self.timeValue)
        
        
        # ComboBox oluşturuluyor
        self.combo_box = QComboBox()
        # ComboBox'a seçenekler ekleniyor
        self.combo_box.addItem("Ayak-Tilt")
        self.combo_box.addItem("Ayak-Gyro")
        self.combo_box.addItem("Boyun-Gyro")
        self.combo_box.addItem("Sıcaklık-Nem")
        self.combo_box.setFont(Font)
        vBoxTimeChoose.addWidget(self.combo_box)
        hBoxIdtimeSituation.addLayout(vBoxTimeChoose)
        ...
        self.butonstartStopTest = QPushButton("Start/Stop Test", self)
        self.butonstartStopTest.clicked.connect(self.startStopTest)
        self.butonstartStopTest.setFixedHeight(screen.height()//7)
        self.butonstartStopTest.setFont(Font)
        self.butonstartStopTest.setStyleSheet("""
            QPushButton {
        ...
        """)
        hBoxButton.addWidget(self.butonstartStopTest) 
        ...
        if 1 <= len(self.ppk2s_connected) <= 2:
            try:
        ...
        self.update_datetime()
         ...
         # Start listener for global keypresses
    ...
    def start_key_listener(self):
        # Listener to capture keypresses globally
    ...
    def on_press(self, key):
        try:
            ...
            if key.char == startStopKey:
                self.butonstartStopTest.click()
                print("startStopTest")
            if key.char == sendDataKey:
                self.buttonTestSituation.click()
                print("sendData")
    ...
    def start_test(self):
        try:
            ...
            if 1 <= len(self.ppk2s_connected) <= 2 and client.is_connected():
                self.x_data=[]
                self.y_data=[]
                self.x_dataMini=[]
                self.y_dataMini=[]
                ...
                if self.combo_box.currentText() == "Ayak-Tilt":
                    self.VEBD=4 #Verinin en büyük değeri
                    self.VEKD=1.5 #Verinin en küçük değeri
                    self.VFD=0.3 # Verinin fark değeri
                    self.servoMove=True
                    self.deviceType=0
                
                if self.combo_box.currentText() == "Ayak-Gyro":
                    self.VEBD=10 #Verinin en büyük değeri
                    self.VEKD=3 #Verinin en küçük değeri
                    self.VFD=0 # Verinin fark değeri
                    self.servoMove=False
                    self.deviceType=0
                
                if  self.combo_box.currentText() == "Boyun-Gyro":
                    self.VEBD=10 #Verinin en büyük değeri
                    self.VEKD=3 #Verinin en küçük değeri
                    self.VFD=0 # Verinin fark değeri
                    self.servoMove=False
                ...
                if  self.combo_box.currentText() == "Sıcaklık-Nem":
                    self.VEBD=2.5 #Verinin en büyük değeri
                    self.VEKD=1.5 #Verinin en küçük değeri
                    self.VFD=0 # Verinin fark değeri
                    self.servoMove=False
                    self.deviceType=2
    ...
    def mqtt_test(self):
        global messages_sending
        if self.mqttstopper and messages_sending:
            if  self.combo_box.currentText() == "Boyun-Gyro":
                if received_messages.find("_Neck")>0:
                    self.pedometreID=received_messages
                    self.pedometreID=self.pedometreID[:8]
    ...
    def update_plot(self):
        try:
            ...
            if self.join>30000 and self.avgValue<self.VEBD:
            ...
            if self.join<30000 and time.time()-self.start_time>15:
                self.situationValue.setText("Join bulunamadı")
                self.situationValue.setPalette(self.paletteRed)
                
            if self.avgValue>self.VEBD and time.time()-self.start_time>15:
        ...
        try:
            ...
            if self.isTestStarted:
                ...
                if len(self.dikKonum1) > 1:
                    ...
                    if self.VEBD > self.dikKonum1[-1] > self.VEKD:
                ...
                if len(self.yatayKonum1) > 1:
                    ...
                    if self.VEBD > self.yatayKonum1[-1] > abs(self.VEKD and self.yatayKonum1[-1] - self.dikKonum1[-1]) >= self.VFD:
                ...
                if len(self.dikKonum2) > 1:
                    ...
                    if self.VEBD > self.dikKonum2[-1] > self.VEKD and abs(self.yatayKonum1[-1] - self.dikKonum2[-1]) >= self.VFD:
                ...
                if len(self.yatayKonum2) > 1:
                    ...
                    if self.VEBD > self.yatayKonum2[-1] > self.VEKD and abs(self.yatayKonum2[-1] - self.dikKonum2[-1]) >= self.VFD:
                ...
                if len(self.dikKonum3) > 1:
                    ...
                    if self.VEBD > self.dikKonum3[-1] > self.VEKD and abs(self.yatayKonum2[-1] - self.dikKonum3[-1]) >= self.VFD:
                ...
                if len(self.yatayKonum3) > 1:
                    ...
                    if self.VEBD > self.yatayKonum3[-1] > self.VEKD and abs(self.yatayKonum3[-1] - self.dikKonum3[-1]) >= self.VFD:
                ...
                if len(self.dikKonumOrt) != 0:
                    ...
                    if self.VEBD > dikKonumOrtValue > self.VEKD:
                ...
                if len(self.yatayKonumOrt) != 0:
                    ...
                    if self.VEBD  > yatayKonumOrtValue > self.VEKD and abs(yatayKonumOrtValue - dikKonumOrtValue) >= self.VFD:
